    ...
    T: Ord + Clone + core::fmt::Debug,
{
    ...
    for i in (0..n).rev() {
        ...
        assert_ne!(
            v_opt, None,
            "tri_par_tas : Erreur interne (1): Il devrait rester des éléments dans le tas."
        );
    ...
    assert_eq!(
        binary_heap.pop(),
        None,
        "tri_par_tas : Erreur interne (2) : Il ne devrait plus rester d'élément dans le tas."
    );
} // fn tri_par_tas
...
where
    T: Ord, // Le type T doit avoir le 'trait' Rust 'Ord': Les éléments doivent être ordonnés donc comparables
{
    let n = mon_tableau.len();
    for i in 0..(n - 1) {
        if mon_tableau[i + 1] < mon_tableau[i] {
            return false;
        }
    }
    true
