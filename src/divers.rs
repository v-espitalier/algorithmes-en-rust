...
pub fn resoud_probleme_des_8_dames() -> Vec<[usize; 8]> {
    ...
    loop {
        ...
        for i in 0..8 {
            solution_absolue_cour[i] =
                trouve_k_ieme_case_libre(solution_relative_cour[i], i, &solution_absolue_cour);
        }
        ...
        if solution_relative_cour[0] > 3 {
            println!(
                "nombre de positions testées avec pruning: {}",
                n_positions_testees
            );
            break;
        }
...
pub fn recherche_premiers(min_n: usize, max_n: usize) -> Vec<usize> {
    ...
    if (min_n <= 2) && (max_n > 2) {
        premiers_trouves.push(2);
    }

    let min_n_impair: usize = if (min_n <= 2) {
        3
    } else {
        min_n + (1 - (min_n % 2))
    };
...
pub fn recherche_premiers_multithreading(
    min_n: usize,
    max_n: usize,
    batch_size: usize,
) -> Vec<usize> {
    ...
    if (batch_size == 0) {
        panic!("Erreur dans recherche_premiers_multithreading: Il faut un batch_size non nul");
    }
    ...
    for batch_index in 0..n_batch {
        ...
        let max_n_batch: usize = if (max_n_batch_complet < max_n) {
            max_n_batch_complet
        } else {
            max_n
        };
        ...
        let _handle = std::thread::spawn(move || {
            ...
            let mut premiers_trouves_batch: Vec<usize> =
                recherche_premiers(min_n_batch, max_n_batch);

            let mut shared_premiers_trouves_batch_val =
                shared_premiers_trouves_batch.lock().unwrap();
