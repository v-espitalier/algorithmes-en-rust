...
fn test_classiques_recherche_lineaire_generique() {
    let mon_tableau: &[i32] = &[5, 10, 3, 7, 15];
    assert_eq!(
        classiques::recherche_lineaire_generique(mon_tableau, 3),
        Some(2)
    );
    assert_eq!(
        classiques::recherche_lineaire_generique(mon_tableau, 7),
        Some(3)
    );
    assert_eq!(
        classiques::recherche_lineaire_generique(mon_tableau, 19),
        None
    );

    let mon_tableau: &[String] = &[
        "aa".to_string(),
        "bb".to_string(),
        "cc".to_string(),
        "dd".to_string(),
    ...
    assert_eq!(
        classiques::recherche_lineaire_generique(mon_tableau, "cc".to_string()),
        Some(2)
    );
    assert_eq!(
        classiques::recherche_lineaire_generique(mon_tableau, "aa".to_string()),
        Some(0)
    );
    assert_eq!(
        classiques::recherche_lineaire_generique(mon_tableau, "ee".to_string()),
        None
    );
}
...
fn test_classiques_recherche_dichotomique() {
    ...
    assert_eq!(
        classiques::recherche_dichotomique(mon_tableau, 17, None, None),
        Some(2)
    );
    assert_eq!(
        classiques::recherche_dichotomique(mon_tableau, 50, None, None),
        Some(6)
    );
    assert_eq!(
        classiques::recherche_dichotomique(mon_tableau, 13, None, None),
        None
    );
}
...
fn test_rationnels() {
    ...
    assert_eq!(
        &r1 + &r2,
        r1_plus_r2,
        "Echec test_rationnels (1) : Addition de références."
    );
    assert_eq!(
        &r1 - &r2,
        r1_moins_r2,
        "Echec test_rationnels (2) : Soustraction de références."
    );
    assert_eq!(
        &r1 * &r2,
        r1_mult_r2,
        "Echec test_rationnels (3) : Multiplication de références."
    );
    assert_eq!(
        &r1 / &r2,
        r1_div_r2,
        "Echec test_rationnels (4) : Division de références."
    );
}
